...
async def set_user_bible(ctx: Context, user_id: int, bible: Bible) -> None:
    await ctx.insert_into(
        table='user_prefs',
        values={'user_id': str(user_id), 'bible_id': bible['id']},
...
async def delete_bible(ctx: Context, command: str) -> None:
    await ctx.delete_from(command, table='bible_versions', where=['command = $1'])
